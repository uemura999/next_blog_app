  ...
  return (
    <article className='shadow my-4' key={article.id}>
            <Link href={`articles/${article.id}`} className='hover:opacity-75'>
                <Image src={`https://source.unsplash.com/collection/1346951/1000x500?sig=${article.id}`} alt='' width={1200} height={300}/>
            </Link>
            <div className='bg-white flex flex-col justify-start p-6'>
                ...
                <Link href={`articles/${article.id}`} className='text-pink-800 hover:text-black'>Read More</Link>
            </div>
    </article>
  )
}
